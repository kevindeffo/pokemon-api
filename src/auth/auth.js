...
function generateToken(userId, email) {
    return jwt.sign({ userId, email }, privateKey, { expiresIn: '1h' });
}
...
module.exports = (req, res, next) => {
    ...
    jwt.verify(token, privateKey, (error, decodedToken) => {
        ...
        if ((req.body.userId && req.body.userId !== userId) || (req.body.email && req.body.email !== userEmail)) {
            const message = "L'identifiant de l'utilisateur ou l'email est invalide";
            return res.status(401).json({ message });
        } else {
            next();
        }
...
// Example usage of token generation
